...
def check_user_status(user_id: int):
    ...
    Returns user data if approved, raises HTTPException if not
    """
    try:
        connection = get_connection()
        cursor = connection.cursor(dictionary=True)
        
        cursor.execute("SELECT status, role, org_id FROM Users WHERE user_id = %s", (user_id,))
        ...
        if user['status'] != 'approved':
            raise HTTPException(
                status_code=403, 
                detail=f"Access denied. Your account is {user['status']}. Please wait for admin approval."
            )
